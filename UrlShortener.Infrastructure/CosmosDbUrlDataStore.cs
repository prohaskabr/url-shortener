...
public class CosmosDbUrlDataStore : IUrlDataStore
{
    ...
    public async Task AddAsync(ShortenedUrl shortened, CancellationToken token)
    {
        var document = (ShortenedUrlCosmos)shortened;
        await _container.CreateItemAsync(document, new PartitionKey(document.PartitionKey), cancellationToken: token);
    }
